...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        if self.use_ema:
            self.model_ema1 = LitEma(self.model1)
            print(f"Keeping EMAs of {len(list(self.model_ema1.buffers()))}.")

            self.model_ema2 = LitEma(self.model2)
            print(f"Keeping EMAs of {len(list(self.model_ema2.buffers()))}.")
...
class LatentDiffusion_m(DDPM):
    ...
    def training_step(self, batch, batch_idx):
        if self.automatic_optimization:
            ...
            loss, loss_dict, t = self.shared_step(batch)
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            print("********************************")
            print('self.split_input_params is True')
    ...
    def configure_optimizers(self):
        lr = self.learning_rate
        params1 = list(self.model1.parameters())
        params2 = list(self.model2.parameters())
        if self.cond_stage_trainable:
            print(f"{self.__class__.__name__}: Also optimizing conditioner params!")
            params1 = params1 + list(self.cond_stage_model.parameters())
        if self.learn_logvar:
            print('Diffusion model optimizing logvar')
            params1.append(self.logvar)
        ...
        if self.use_scheduler:
            ...
            scheduler = [
                {
                    'scheduler': LambdaLR(opt1, lr_lambda=scheduler.schedule),
                    'interval': 'step',
                    'frequency': 1
                }]
...
class Layout2ImgDiffusion(LatentDiffusion_m):
